...
HEOB_VERSION:=4.1-dev-$(DATE)
HEOB_VER_NUM:=4,1,0,99
HEOB_PRERELEASE:=1
...
test: heob$(BITS).exe allocer$(BITS).exe
	@$(foreach t,$(TESTS),echo test$(BITS)-$(t): heob$(BITS) $(T_H$(t)) allocer$(BITS) $(T_A$(t)) "->" `./heob$(BITS).exe $(T_H$(t)) allocer$(BITS) $(T_A$(t)) </dev/null |sed 's/0x[0-9A-Z]*/0xPTR/g;/^[ |]*0xPTR/d;s/\<of [1-9][0-9]*/of NUM/g;s/^[ |]*\[/    \[/;s/ \[[0-9]\+\]//;s/ '\''1'\''//;/^           *[^\[]/d' |diff -Naur --strip-trailing-cr --label "expected result" --label "actual result" testres/test-$(t).txt - >test$(BITS)-$(t).diff && echo "$(TOK)" && rm -f test$(BITS)-$(t).diff || echo "$(TFAIL)"`;)
