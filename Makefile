...
HEOB_VERSION:=4.2-dev-$(DATE)
HEOB_VER_NUM:=4,2,0,99
HEOB_PRERELEASE:=1
HEOB_COPYRIGHT_YEARS:=2014-2025
...
WARN=-Wall -Wextra -Wshadow -Wwrite-strings -Werror \
...
WARN_TEST=-Wno-stringop-overflow -Wno-maybe-uninitialized \
	  -Wno-use-after-free -Wno-stringop-overread
CFLAGS_TEST=$(WARN) $(CFLAGS) -O3 -g -D_GLIBCXX_INCLUDE_NEXT_C_HEADERS \
	    -D__USE_MINGW_ANSI_STDIO=0 \
	    $(NO_THREAD_JUMPS) $(WARN_TEST) \
	    -Wno-uninitialized -Wno-array-bounds \
	    -Wno-free-nonheap-object -Wno-mismatched-new-delete
...
T_A61=23
T_H62=-p1 -a16 -f0 -g0 -T1
T_A62=26
T_H63=-p1 -a4 -f0 -z50
T_A63=1
T_H64=-p1 -a16 -f0 -g2 -T1
T_A64=26
...
testc: heob$(BITS).exe allocer$(BITS).exe | testres
	@$(foreach t,$(TESTS),echo heob$(BITS) $(T_H$(t)) allocer$(BITS) $(T_A$(t)) "->" test$(BITS)-$(t).txt; ./heob$(BITS).exe $(T_H$(t)) allocer$(BITS) $(T_A$(t)) |sed 's/0x[0-9A-Z]*/0xPTR/g;/^[ |]*0xPTR/d;s/\<of [1-9][0-9]*/of NUM/g;s/^[ |]*\[/    \[/;s/ \[[0-9]\+\]//;s/ '\''1'\''//;/^           *[^\[]/d' >testres/test-$(t).txt;)
